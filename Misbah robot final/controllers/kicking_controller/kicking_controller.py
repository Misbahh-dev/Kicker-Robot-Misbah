"""
Robot Kicker – ALIGN_BALL uses ball centre to sidestep
"""
import cv2
import numpy as np
from controller import Robot, Motion

# Colour thresholds and constants
...
class NaoGoalTracker(Robot):
    ...
    def _state_align_ball(self, ball_detected, ball_cx):
        if not ball_detected:
            print('[NaoGoalTracker] Ball lost during align')
            return
        if BALL_CENTRE_MIN <= ball_cx <= BALL_CENTRE_MAX:
            print(f'[NaoGoalTracker] Ball centred (cx={ball_cx:.3f})')
            # Next commit will add APPROACH
            return
        if self._is_busy():
            return
        error = ball_cx - BALL_CENTRE_MID
        if error > 0:
            print(f'[NaoGoalTracker] Ball RIGHT (cx={ball_cx:.3f}) -> sidestep right')
            self._play_motion(self.mot_side_right)
        else:
            print(f'[NaoGoalTracker] Ball LEFT  (cx={ball_cx:.3f}) -> sidestep left')
    ...
    def run(self):
        while self.step(self.timeStep) != -1:
            ...
            ball_detected, ball_cx, _ = detect_ball(bot_raw, self.cam_w_bot, self.cam_h_bot)
            if ball_detected:
